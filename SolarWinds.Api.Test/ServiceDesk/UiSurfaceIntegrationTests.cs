...
public class UiSurfaceIntegrationTests(ITestOutputHelper output) : TestWithOutput(output)
{
	...
	public async Task UiInfrastructure_Endpoints_ReturnResults()
	{
		...
		try
		{
			await AssertCustomContextAndUrlFiltersAsync();
			await AssertDashboardsAndWidgetsAsync();
			await AssertSessionAssetsAsync();
			await AssertListAndSetupSurfacesAsync();
		}
		catch (ApiException ex) when ((int)ex.StatusCode == 401)
		{
			// Some UI infrastructure endpoints can require additional web-session context, so a 401
			// here is treated as 'not available on this tenant' rather than a failure.
		}
	}

	private async Task AssertCustomContextAndUrlFiltersAsync()
	{
	...
	/// <summary>
	/// Reads the dashboard list, and the per-dashboard and widget endpoints that only make sense
	/// on a tenant that has at least one dashboard.
	/// </summary>
	private async Task AssertDashboardsAndWidgetsAsync()
	{
		...
		if (dashboards.Count == 0 || dashboards[0].Id is not int dashboardId)
		{
			return;
		}
	...
	/// <summary>
	/// Reads the assets the portal front end fetches for a session: its websocket token and its
	/// real-user-monitoring script.
	/// </summary>
	private async Task AssertSessionAssetsAsync()
	{
	...
	private async Task AssertListAndSetupSurfacesAsync()
	{
