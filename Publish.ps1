...
# Write-Host cannot be captured, suppressed or redirected by a caller, so progress messages go
# through Write-Information instead. Emphasis is applied with $PSStyle where the host provides it,
# which keeps the failure messages as easy to spot as -ForegroundColor made them.
function Write-Status {
	...
	param(
		...
		[ValidateSet('None', 'Red', 'Yellow', 'Cyan', 'Green')]
		[string]$Colour = 'None'
	)

	$text = $Message
	if ($Colour -ne 'None' -and $PSStyle) {
